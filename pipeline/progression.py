...
SENTENCE_TOLERANCE = 5     # target ± this many sentences (raised 1→2→3→5 on
                           # 2026-04-24 so reinforcement-weave sentences
                           # don't force a re-pagination cascade. The old ±1
                           # was tuned when stories were generated by an LLM
                           # at a fixed length; with deterministic authoring
                           # and library-wide reinforcement weaving, prose
                           # length is set by what the curriculum needs to
                           # reinforce, not by an editorial target.)
CONTENT_LOW = 0.5          # target × this is the minimum content-token count
...
CONTENT_HIGH = 2.0         # target × this is the maximum content-token count
