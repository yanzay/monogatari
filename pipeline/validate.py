...
def validate(
    ...
    if sid_for_tier and sid_for_tier > 0:
        try:
            ...
            if STORIES.is_dir():
                ...
                # corresponding library-level pytest. The in-validator branch
                # has been off since the converter switched to honest
                # library-wide first-occurrence semantics.
        except ImportError:
            # grammar_progression module missing — soft-skip (back-compat)
            pass
    ...
    # Check 3.7 (vocab cadence floor) is now enforced solely by
    # `pipeline/tools/cadence.py vocab-cadence` and its pytest peer; the
    # in-validator branch has been off since the converter switched to
    # honest library-wide first-occurrence semantics.
