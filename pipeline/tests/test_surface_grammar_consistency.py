...
that の had been silently mis-tagged as G001_wa_topic in 22 places, な
as G003_desu in 3 places, and で as G004_ni_location in 1 place.
...
FORBIDDEN_PAIRS: dict[tuple[str, str], set[str]] = {
    ("particle", "の"): {"G001_wa_topic"},     # の is possessive (G015), not topic
    ("particle", "な"): {"G003_desu"},         # な is na-adj attributive (G016), not copula
    # Note: で can be G004_ni_location ONLY when it's actually に misspelled — but it never is.
    # で should always be G017_de_means in our scope.
    ("particle", "で"): {"G004_ni_location"},
}
...
EXPECTED_PAIRS: dict[tuple[str, str], str] = {
    ("particle", "の"): "G015_no_possessive",
    ("particle", "な"): "G016_na_adjective",
    ("particle", "で"): "G017_de_means",
    ...
    # です/だ/でした collapse into G003_desu / G013_mashita_past — handled separately
    ("aux", "そして"): "G012_soshite_then",
    ("particle", "そして"): "G012_soshite_then",
}
...
POLYSEMOUS: dict[tuple[str, str], set[str]] = {
    ...
    ("aux", "ます"): {"G003_desu", "G026_masu_nonpast"},  # politeness aux
    ("aux", "ました"): {"G013_mashita_past"},
}
